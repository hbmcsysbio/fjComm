...
inline void count_gk_curr(int &k,size_t &kmerPerLig,unsigned &currLigLen,const vector<unsigned long> &k_code_currLig,vector<IntegerVector> &allkShiftCombis,IntegerVector allGapCombiLens,arma::umat &kcnt)
  {
...
)
{
  ...
  arma::umat kcnt(dim_k, gapCombi);  kcnt.fill(pseudo);
  ...
  if (all_possible_k) //retain all rows or only exist
    {
  ...
  NumericMatrix result= wrap(kcnt);
  result.attr("dimnames")= List::create(rowNames_k,colNames_g);
