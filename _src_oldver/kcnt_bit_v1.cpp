...
inline void count_gk_curr(int &k,size_t &kmerPerLig,unsigned &currLigLen,const vector<unsigned long> &k_code_currLig,vector<IntegerVector> &allkShiftCombis,IntegerVector allGapCombiLens,NumericMatrix &kcnt)
  {
    for (size_t i=0; i< kmerPerLig;i++) //cnt gk for each pos
    {
      size_t &currPos=i;
      ...
      for (size_t j=0; j<combiNo; j++) //loop through gap combis
      {
        ...
        // cout<< currPos << "  combiNo:"<<j<<"  shift1:"<< allkShiftCombis[j][0]<< "  shift2:"<< allkShiftCombis[j][1] << "  sumOfLen"<<allGapCombiLens[j]+currPos<< "  ligLen:"<<currLigLen<< "\n";
        // cout<< "   kmerCode:"<< combi_k_code <<"\n";
        kcnt(combi_k_code,j)++; //[currPos]
        end_curr_gap_combi:;
      }
    }
  }
...
SEXP gkmerCntBit(std::vector<std::string> strings,
                 ...
                 bool all_possible_k=true, bool melt=false
)
{
  if (gapMaxs.size()!=gapNo || gapMins.size()!=gapNo) stop("gapMaxs and gapMins should be the same lens as gapNo"); //k.size()!= gapNo+1
  ...
  unsigned long gapCombi= allGapCombis.size();  //1; for(int i : gapRng) gapCombi*=i;
  unsigned long dim_k=pow(4,kLenSum);
  NumericMatrix kcnt (dim_k, gapCombi); // for result
  std::fill(kcnt.begin(), kcnt.end(), pseudo);
  ...
  for (unsigned i = 0; i < ligNo; i++)
  {
    ...
    count_gk_curr(k,kmerPerLig,ligLens,k_code_currLig,allkShiftCombis,allGapCombiLens,kcnt);

  }


  unsigned kNum= kcnt.nrow();
  ...
  kcnt.attr("dimnames")= List::create(rowNames_k,colNames_g);

  // if(!all_possible_k) //only return appeared
  //   {
  //     // NumericMatrix exist;
  //     for(size_t i=0; i<kcnt.nrow();i++)
  //       if(sum(kcnt(i,))==0) kcnt.erase()
  //   }
  // if(melt){}
