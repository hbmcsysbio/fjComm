...
knit_oneOff_source<-function(Rfile="xx.R",Overwrite=FALSE)
{
  system("mkdir -p oneoff/env/")
  env_file_name="oneoff/env/" %>% paste0(Rfile,".env")
  if(file.exists(env_file_name) && Overwrite==FALSE) {load(env_file_name,.GlobalEnv) }else{source(Rfile);  save.image(env_file_name);load(env_file_name,.GlobalEnv)}
  knit_ini()
}

# clear and set default block param
knit_ini<- gtools::defmacro(var,expr={
